...
class ModelClass(DeepProfilerModel):
    def __init__(self, config, crop_generator, val_crop_generator, is_training):
        super(ModelClass, self).__init__(config, crop_generator, val_crop_generator, is_training)
        self.feature_model, self.optimizer, self.loss = self.define_model(config)
    ...
    def define_model(self, config):
        ...
        if "use_pretrained_input_size" in config["profile"].keys() and self.is_training is False:
            ...
            y = tf.keras.layers.Dense(self.config["num_classes"], activation="softmax", name="ClassProb")(
                features)
            class_outputs.append(y)
            # 4. Create and compile model
            model = tf.keras.models.Model(inputs=input_image, outputs=class_outputs)
    ...
    def copy_pretrained_weights(self):
        ...
        lshift = self.feature_model.layers[
                     1].name == 'augmentation_layer'  # Shift one layer to accommodate the AugmentationLayer
