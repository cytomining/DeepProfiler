...
def define_model(config, dset):
    ...
    model = DenseNet(input_shape=input_shape, depth=config["train"]["model"]["params"]["conv_blocks"],
                     classes=dset.targets[0].shape[1], input_tensor=input_image)  #freeze_bn=not is_training)
