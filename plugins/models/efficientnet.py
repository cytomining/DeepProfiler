...
class EfficientNetMixin(object):
    # Architecture methods shared by the v1 (graph) and v2 (eager) base classes.
    ...
    def get_model(self, config, input_image=None, weights=None, include_top=False):
        ...
        if self.is_training and config["dataset"]["locations"]["mode"] == "full_image" and input_image is not None:
            ...
            boxes = tf.keras.layers.Input(shape=(4,))
            box_ind = tf.keras.layers.Input(shape=(), dtype="int32")
        ...
        if self.is_training and config["dataset"]["locations"]["mode"] == "full_image" and input_image is not None:
            model = tf.keras.Model(inputs=[input_view, boxes, box_ind], outputs=model.output)
    ...
    def define_model(self, config, dset):
        ...
        loss_func = tf.keras.losses.CategoricalCrossentropy(label_smoothing=
        ...
        if not self.is_training and "use_pretrained_input_size" in config["profile"].keys():
            ...
            features = tf.keras.layers.GlobalAveragePooling2D(name="pool5")(model.layers[-1].output)
            # 2. Create an output embedding for each target
            class_outputs = []

            y = tf.keras.layers.Dense(self.config["num_classes"], activation="softmax", name="ClassProb")(features)
            class_outputs.append(y)

            # 4. Create and compile model
            model = tf.keras.Model(inputs=model.input, outputs=class_outputs)
            ...
            if self.config["train"]["model"].get("augmentations") is True:
                model = tf.keras.models.model_from_json(
                    model.to_json(),
                    {'AugmentationLayer': self.augmentation_base}
                )
            else:
...
class ModelClass(EfficientNetMixin, DeepProfilerModel):
    def __init__(self, config, dset, crop_generator, val_crop_generator, is_training):
        ...
        self.augmentation_base = AugmentationLayer()
