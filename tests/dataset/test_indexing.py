...
def test_write_compression_index():
    temp = os.path.dirname("DeepProfiler/deepprofiler/examples/metadata/tmp/")
    if os.path.exists(temp) == False:
        os.makedirs(temp)
    open_file = open("DeepProfiler/deepprofiler/examples/config/test_indexing.json")
    config = json.load(open_file)
    deepprofiler.dataset.indexing.write_compression_index(config)
    test_output = pd.DataFrame.from_csv("DeepProfiler/deepprofiler/examples/metadata/tmp/index.csv")
...
def test_split_index():
    ...
    config = json.load(open_file)
    test_parts = 3
    test_paths = ["DeepProfiler/deepprofiler/examples/metadata/tmp/index-000.csv",
                  "DeepProfiler/deepprofiler/examples/metadata/tmp/index-001.csv",
                  "DeepProfiler/deepprofiler/examples/metadata/tmp/index-002.csv"]
    deepprofiler.dataset.indexing.write_compression_index(config)
    ...
    test_outputs = [pd.DataFrame.from_csv("DeepProfiler/deepprofiler/examples/metadata/tmp/index-000.csv"),
                    pd.DataFrame.from_csv("DeepProfiler/deepprofiler/examples/metadata/tmp/index-001.csv"),
                    pd.DataFrame.from_csv("DeepProfiler/deepprofiler/examples/metadata/tmp/index-002.csv")]
