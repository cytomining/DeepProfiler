...
def metadata(out_dir):
    ...
    df = pd.DataFrame({
        ...
        'Target': [None, None, None, None, None, None, None, None, None, None, None, None]  # TODO: rewrite autoencoder crop generator so that this is not necessary
    }, dtype=int)
...
def test_autoencoder_crop_generator():
    assert issubclass(plugins.crop_generators.crop_generator.GeneratorClass, deepprofiler.imaging.cropping.CropGenerator)
    assert issubclass(plugins.crop_generators.crop_generator.SingleImageGeneratorClass, deepprofiler.imaging.cropping.SingleImageCropGenerator)
...
def test_single_image_generator_class_generate(config, dataset, tmpdir):
    ...
    path = os.path.join(path,
                        '{}-{}-{}.csv'.format(meta['Metadata_Well'],
                                              meta['Metadata_Site'],
                                              crop_generator.config['sampling']['locations_field']))
    locations = pd.DataFrame({
        'R_Location_Center_X': np.random.randint(0, 128, (crop_generator.config['sampling']['locations'])),
        'R_Location_Center_Y': np.random.randint(0, 128, (crop_generator.config['sampling']['locations']))
    })
